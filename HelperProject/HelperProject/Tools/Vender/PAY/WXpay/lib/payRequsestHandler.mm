...
- ( NSMutableDictionary *)sendPay_demo
{
    ...
    [packageParams setObject: _body        forKey:@"body"];         //订单描述，展示给用户
    [packageParams setObject: _detail        forKey:@"detail"];  
    [packageParams setObject: @"APP-0013"        forKey:@"device_info"]; //支付设备号或门店号
    [packageParams setObject: mchid             forKey:@"mch_id"];      //商户号
    [packageParams setObject: noncestr          forKey:@"nonce_str"];   //随机串
    [packageParams setObject: _notifyurl        forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: _outtradeno           forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: [ALHardware localIPAddress]    forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: @"APP"            forKey:@"trade_type"];  //支付类型，固定为APP
    [packageParams setObject: _price       forKey:@"total_fee"];       //订单金额，单位为分
